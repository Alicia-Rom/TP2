...
def build_arg_parser():
    ...
    parser.add_argument("--steering-calibration", type=float, default=0.15)
    parser.add_argument(
        "--steering-gain",
        type=float,
        default=1.25,
...
def main():
    ...
    while True:
        ...
        manual_turn_active = (
            not decision.stop
            and decision_engine.direction_control in (1, 3)
            and now < decision_engine.direction_until
        )
        if manual_turn_active:
            control_giro, control_acelerador = controls_without_road(
                decision_engine.direction_control,
