...
DEFAULT_MIN_DETECTION_AREA = 1100.0
...
def detection_passes_distance_filter(detection):
    ...
    if DEFAULT_MIN_DETECTION_WIDTH > 0 and (width is None or width < DEFAULT_MIN_DETECTION_WIDTH):
        return False

    if DEFAULT_MIN_DETECTION_HEIGHT > 0 and (height is None or height < DEFAULT_MIN_DETECTION_HEIGHT):
        return False

    if DEFAULT_MIN_DETECTION_AREA > 0 and area < DEFAULT_MIN_DETECTION_AREA:
        return False

    if DEFAULT_MIN_DETECTION_CENTER_Y > 0 and (
        center_y is None or center_y < DEFAULT_MIN_DETECTION_CENTER_Y
    ):
        return False
...
def format_filter_settings():
    ...
    # que umbrales de cercania estan activos para las senales.
    parts = []
    if DEFAULT_MIN_DETECTION_AREA > 0:
        parts.append(f"area>={DEFAULT_MIN_DETECTION_AREA:.0f}")
    if DEFAULT_MIN_DETECTION_WIDTH > 0:
        parts.append(f"width>={DEFAULT_MIN_DETECTION_WIDTH:.0f}")
    if DEFAULT_MIN_DETECTION_HEIGHT > 0:
        parts.append(f"height>={DEFAULT_MIN_DETECTION_HEIGHT:.0f}")
    if DEFAULT_MIN_DETECTION_CENTER_Y > 0:
        parts.append(f"center_y>={DEFAULT_MIN_DETECTION_CENTER_Y:.0f}")
