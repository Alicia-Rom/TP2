...
def build_arg_parser():
    ...
    #Esto luego se multiplica por steering_gain, asi que se recomienda configurar valores menores a 1 para evitar giros excesivos.
    parser.add_argument(
    ...
    parser.add_argument(
        "--turn-maneuver-seconds",
        type=float,
        default=1.8,
        help="Tiempo durante el que una senal de giro bloquea la busqueda de trayectoria.",
    )
...
def main():
    ...
    print(f"Maniobra de giro sin trayectoria: {args.turn_maneuver_seconds:.2f}s")
    print(
    ...
    manual_turn_until = 0.0
    manual_turn_control = 0
    ...
    while True:
        ...
        if decision.stop:
            manual_turn_until = 0.0
            manual_turn_control = 0
        elif decision.source_label and control_mode in (1, 3):
            manual_turn_control = control_mode
            manual_turn_until = now + max(args.turn_maneuver_seconds, 0.0)

        manual_turn_active = now < manual_turn_until and manual_turn_control in (1, 3)
